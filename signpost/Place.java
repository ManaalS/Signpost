        ...
        for (int x = 0; x <= width - 1; x += 1) {
            for (int y = 0; y <= height - 1; y += 1) {
                ...
                for (int direction = 1; direction <= 8; direction += 1) {
                    M[x][y][direction] = new PlaceList();
                    for (int j = 1; j < lim; j++) {
                        int xnew = j * (dx(direction)) + x;
                        int ynew = j * (dy(direction)) + y;
                        boolean cond1 = (0 <= xnew && xnew < width);
                        boolean cond2 = (0 <= ynew && ynew < height);
                        if (cond1 && cond2) {
                            M[x][y][direction].add(pl(xnew, ynew));
                            M[x][y][0].add(pl(xnew, ynew));
                        }
                    }
                }
            }
